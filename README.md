...
- **Linearization of Power Flow Equations:**
   ...
   ###### *Note: As a consequence of the discretisation of the problem, hourly time intervals are used instead of continuous time. For consistency, in the following, the powerflow* $e_{i,j,t}$ *will be expressed in units of energy [kwh] (power by hour).*
...
11. **Turn on / Turn off delays (only if node type is $p = \text{thermal}$):**
   - Turning off delay after turning on
      ...
      - $\sum_{t+1 \leq k \leq t+TURNOFFDELAY_i} off_{i,k} \leq (1-on_{i,k}) \cdot M \quad2 \forall i, \forall t \leq max(Time) - TURNOFFDELAY_i$
     
   - Tunring on delay after turning off
     
      - $\sum_{t+1 \leq k \leq t+TURNONDELAY_i} on_{i,k} \leq (1-off_{i,k}) \cdot M \quad2 \forall i, \forall t \leq max(Time) - TURNONDELAY_i$
...
2. **Execute Model**
   ...
   Open the `main.py` script and adjust its global parameters (defined below the package import section). This parameters allow to set the solver for optimisation of the model and to activate the generation of a video showing the optimal evolution over time of the network after finishing the optimisation process.
